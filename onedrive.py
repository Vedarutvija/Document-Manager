            ...
            try:
                ...
                if file_type == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document':
                    ...
                    if text_content:
                        ...
                        metadatas = [{"source": item_path, "file_name": file_name, "file_id": item_id, "chunk_index": i, "processing_date": datetime.now().isoformat()} for i in range(len(chunks))]
...
llm = ChatOpenAI(openai_api_key=OPENAI_API_KEY, temperature=0.7, max_tokens=150)
...
@app.route('/')
def index():
    return redirect(url_for('login')) if not session.get("user") else render_template('index.html')
...
def authorized():
    if request.args.get('code'):
        token_result = msal_app.acquire_token_by_authorization_code(request.args['code'], scopes=SCOPE, redirect_uri=REDIRECT_URI)
...
def upload_file():
    ...
    try:
        ...
        chunks = text_splitter.split_text(text_content)
        metadatas = [{"source": f"Documents/{filename}", "file_name": filename, "file_id": file_id, "chunk_index": i, "upload_date": datetime.now().isoformat()} for i in range(len(chunks))]
        vectorstore.add_texts(texts=chunks, metadatas=metadatas, ids=[f"{file_id}_chunk_{i}" for i in range(len(chunks))])

        return jsonify({'success': True, 'message': 'File uploaded and processed successfully', 'file_id': file_id, 'file_name': filename, 'chunks_processed': len(chunks)})
